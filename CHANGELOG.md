...
| `install.sh` | 加 `--userscripts-only` flag(只跑 Step 1/2/4,跳过 symlink scripts / template / base / rules / quickadd snippet) |
| `CHANGELOG.md` | 加本段 |
| `README.md` | badge bump v0.8.0 → v0.8.1 |

### 🆕 install.sh `--userscripts-only` flag

**问题**:仓库 quickadd userscripts 改了之后,vault 内是 install.sh `cp + sed inject` 装出来的物理文件(不是 symlink,见 v0.3.4 改 cp 原因),仓库改动不会自动同步。原本只能 `install.sh --apply --force` 全量重装,但这会顺手覆盖 `task 模版.md` / `_task.base` / `feishu-project-sync.md` 等用户可能改过的资产,引来副作用。

**解决**:加 `--userscripts-only` flag,只跑 Step 1(依赖检查)+ Step 2(vault 路径)+ Step 4(cp + sed userscripts),Step 3 / 5 / 6 / 7 全跳过。

用法:
```bash
./install.sh --userscripts-only --vault-path <vault> --scripts-dir <vault相对路径> --apply --force
```

注:`--apply --force` 必须同时给,否则 Step 4 会因 `已存在 → 跳过` 不做事(`--force` 控制覆盖)。

**对老 dogfood 实例**:本次先用 Python 手脚本干等价的 `cp + sed inject` 升级了 7 个 userscripts(因为 flag 还没加),v0.8.1 后此 flag 是官方升级路径。
...
| 6 | 人可读 | OK |
| 7 | 高复用 | 同 #1,模板冗余暂未抽象 |
| 8 | 工程化 | install.sh `cp + sed` 物理文件 vs symlink 的 trade-off 是已知决策(v0.3.4 因 __filename 失败而改 cp),本次暴露了「仓库改了不会自动同步到 vault」的副作用 → **本版已落地 `install.sh --userscripts-only` flag**,以后单独同步 userscripts 一条命令搞定(不动 template/base/rules)|
