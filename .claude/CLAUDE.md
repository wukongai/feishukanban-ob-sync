...
- vault 内 sync.py(symlink → 仓库):`01 Project/00 进行中/06 小工具开发/CC命令/飞书项目同步/sync.py`
- vault 内 config.yaml(symlink → 仓库 config.yaml):同上目录
- vault 内 QuickAdd choices:`.obsidian/plugins/quickadd/data.json`(4 个 choices 指向 `scripts/feishukanban-ob-sync/userscripts/`)
