...
today_source:                      # planned / unplanned / 空(最近一次来源)
# v0.7.2: today_source_history 与 today_history 一一对应的来源快照(按天)
# - 与 today_history 长度对齐:today_history[i] 对应 today_source_history[i]
# - 只追加,不回溯改 → 解决"6-01 unplanned task 经 6-02 pull-today 后历史失真"问题
# - OB-only(不同步飞书);Dataview 渲染历史日时按这个字段判断当天到底是 planned 还是 unplanned
today_source_history:              # YAML inline list,如 [unplanned, planned]
